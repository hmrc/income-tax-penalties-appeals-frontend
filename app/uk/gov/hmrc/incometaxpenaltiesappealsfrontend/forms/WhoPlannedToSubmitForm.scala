...
object WhoPlannedToSubmitForm extends Mappings {

  val key = "whoPlannedToSubmit"

  def form()(implicit messages: Messages): Form[AgentClientEnum.Value] = Form[AgentClientEnum.Value](
    single(
      key -> text(messages("agents.whoPlannedToSubmit.error.required"))
        .verifying(messages("agents.whoPlannedToSubmit.error.invalid"), x => Try(AgentClientEnum.withName(x)).isSuccess)
