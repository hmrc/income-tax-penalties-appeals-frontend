...
import uk.gov.hmrc.incometaxpenaltiesappealsfrontend.models.upscan.{FailureDetails, FailureReasonEnum, UploadDetails, UploadFormFields, UploadJourney, UploadStatusEnum, UpscanInitiateRequest, UpscanInitiateResponse}
...
trait FileUploadFixtures extends BaseFixtures {
  ...
  val uploadFields: Map[String, String] = Map(
    "key" -> "abcxyz",
    "algo" -> "md5"
  )
  ...
  val initiateResponse: UpscanInitiateResponse = UpscanInitiateResponse(
    reference = fileRef1,
    uploadRequest = UploadFormFields(
