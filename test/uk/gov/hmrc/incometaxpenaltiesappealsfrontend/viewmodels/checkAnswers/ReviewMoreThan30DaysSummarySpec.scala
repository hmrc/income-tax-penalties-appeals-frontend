    ...
    Seq(ReviewMoreThan30DaysMessages.English, ReviewMoreThan30DaysMessages.Welsh).foreach { messagesForLanguage =>
      ...
      s"being rendered in lang name '${messagesForLanguage.lang.name}'" when {
        ...
        "there's no answer" should {
          ...
          "return None" in {
            implicit val request: CurrentUserRequestWithAnswers[_] = userRequestWithAnswers(emptyUserAnswersWithLSP2ndStage)
        ...
        "there's an answer" when {
          ...
          "show action links is set to true" should {
            ...
            "must output the expected row with a change link" in {
              ...
              ReviewMoreThan30DaysSummary.row() shouldBe Some(summaryListRow(
                ...
                actions = Some(Actions(
                  items = Seq(
                    ActionItem(
                      ...
                      href = controllers.routes.ReviewMoreThan30DaysController.onPageLoad(isAgent, CheckMode).url,
          ...
          "show action links is set to false" should {
            ...
            "must output the expected row WITHOUT a change link" in {
              ...
              ReviewMoreThan30DaysSummary.row(showActionLinks = false) shouldBe Some(summaryListRow(
                label = messagesForLanguage.cyaKey,
                value = Html(messagesForLanguage.yes),
                actions = None
              ))
