  ...
  def row(showActionLinks: Boolean = true)(implicit user: CurrentUserRequestWithAnswers[_], messages: Messages): Option[SummaryListRow] =
    ReasonableExcusePage.value.flatMap { reasonableExcuse =>
      HasHospitalStayEndedPage.value.map { stayEnded =>
        summaryListRow(
          label = messages(s"checkYourAnswers.hasHospitalStayEnded.$reasonableExcuse.key"),
          value = Html(messages(if(stayEnded) "common.yes" else "common.no")),
          actions = Option.when(showActionLinks)(Actions(
            items = Seq(
              ActionItem(
                content = Text(messages("common.change")),
                href = controllers.routes.HasHospitalStayEndedController.onPageLoad(isAgent = user.isAgent, CheckMode).url,
