...
private[mappings] class LocalDateFormatter @Inject()(timeMachine: TimeMachine)(invalidKey: String,
    ...
    parseResult.fold(
      ...
      date => {
        if (futureKey.isDefined) {
          ...
          } else if (date.isEqual(timeMachine.getCurrentDate) || date.isBefore(timeMachine.getCurrentDate)) {
            Right(date)
          } else {
