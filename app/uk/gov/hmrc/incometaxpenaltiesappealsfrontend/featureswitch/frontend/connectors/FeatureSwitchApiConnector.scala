...
package uk.gov.hmrc.incometaxpenaltiesappealsfrontend.featureswitch.frontend.connectors
      ...
      .withBody(Json.toJson(featureSwitchSettings)).execute.map {
        response =>
          response.status match {
            ...
            case _ => throw new Exception(s"Could not update feature switches from $featureSwitchProviderUrl")
          }
      }
  }
}
